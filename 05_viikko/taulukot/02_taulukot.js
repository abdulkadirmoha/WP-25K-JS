...
planets.forEach((planet, index) => {
    console.log(`${planet}, indeksi: ${index}`)
})
...
mixedValues.forEach((values, index, ) =>{
    console.log(
      `Alkio: ${values}, Indeksi: ${index}, Tyyppi: ${typeof values}`
    );
...
*/
// Kirjoita koodisi tähän
const matched = elainvarasto1.filter(item => elainvarasto2.includes(item));
console.log(`Yhteinen laji: ${matched}`);
...
*/
// Kirjoita koodisi tähän
positiveTemperatures = [...temperatures].filter((temp)=>{
    return temp >= 0;
})
...
*/
// Kirjoita koodisi tähän
const etsiEläin = (eläin) =>{
    let eläinIndex= zooAnimals.indexOf(eläin);
    if (eläinIndex == -1) {
      console.log(`${eläin} ei ole eläintarhassa`);
...
if(kirahviIndex !== -1){
    console.log((kirahviIndex = "Alpakka"));
}
...
attendees.forEach((name, index) => {
    if(name === "Jonna"){
        attendeesIndex.push(index);
    }
})
console.log(attendeesIndex);
...
*/
// Kirjoita koodisi tähän
let osallistujatIsolla = attendees.map((name) => name.toUpperCase());
...
*/
// Kirjoita koodisi tähän
console.log(`${forestAnimals}  ${farmAnimals}`);
...
*/
// Kirjoita funktiosi tähän
const discountprice = prices.map(price => price * 0.10 + price);
...
*/
// Kirjoita funktiosi tähän
const uusiTaulukko = words.filter(word => word.length > 5)
